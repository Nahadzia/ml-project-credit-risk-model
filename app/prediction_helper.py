...
def prepared_df(age,income,loan_amount,loan_tenure_months,avg_dpd_per_delinquency,
...
def calculate_credit_score(input_df,base_score=300,scale_length=600):
    x=np.dot(input_df.values,model.coef_.T) + model.intercept_          # m1*x1+...+b

    # Apply the logistic function to calculate the probability
    default_probability = 1 / (1+np.exp(-x))
    non_defualt_probability=1 - default_probability

    # Convert the probability to a credit score, scaled to fit within 300 to 900
    
    credit_score=base_score + non_defualt_probability.flatten() * scale_length
...
def predict(age,income,loan_amount,loan_tenure_months,avg_dpd_per_delinquency,
    ...
    input_df=prepared_df(age,income,loan_amount,loan_tenure_months,avg_dpd_per_delinquency,
           delinquency_ratio,credit_utilization_ratio,num_open_accounts,residence_type,
           loan_purpose,loan_type)
